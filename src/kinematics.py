...
def std(x, xmean, weights):
    num = x.size
    return np.sqrt(np.sum((x-xmean)**2 * weights) / np.sum(weights) * num / (num-1))
...
def ValueAtRadius(pos, value, radius, rwidth, rlim, ez, zi_weight=True, line_weight=None):
    ...
    if n > 1:
        ...
        if np.sum(weights) != 0:
            ...
            std_vrot = np.sqrt(n/(n-1)*np.sum(weights*(value[idx]-mean_vrot)**2)/np.sum(weights))
        else:
            mean_vrot = 0.
            std_vrot = 0.
    elif n == 1:
...
def Vmax(vrot_mean, vrot_std=None, rbins=None):
    ...
    if isinstance(rbins, (abc.Sequence, np.ndarray)) and isinstance(vrot_std, (abc.Sequence, np.ndarray)):
